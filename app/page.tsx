...
export default function Home() {
  ...
  const [jobsAcrossCountries, setJobsAcrossCountries] = useState(0);
  ...
  const refresh = async () => {
    ...
    const allCountryParams = new URLSearchParams(params); allCountryParams.delete("country");
    const [jobsResponse, allCountriesResponse, sourcesResponse, retryResponse] = await Promise.all([fetch(`/api/jobs?${params}`), fetch(`/api/jobs?${allCountryParams}`), fetch("/api/sources"), fetch("/api/retry/linkedin")]);
    if (jobsResponse.ok) setJobs((await jobsResponse.json()).jobs);
    if (allCountriesResponse.ok) setJobsAcrossCountries((await allCountriesResponse.json()).jobs.length);
  ...
  const countryExcludesResults = Boolean(country && jobs.length === 0 && jobsAcrossCountries > 0);
  ...
  return <main>
    ...
    <section className="search-tools" aria-label="Job search"><label>Job keyword<select value={presetKeyword ? search : "__custom__"} onChange={(event) => setSearch(event.target.value === "__custom__" ? "" : event.target.value)}>{keywordOptions.map((keyword) => <option key={keyword} value={keyword}>{keyword}</option>)}<option value="__custom__">Custom keyword…</option></select></label>{!presetKeyword && <label>Custom keyword<input autoFocus value={search} onChange={(event) => setSearch(event.target.value)} placeholder="e.g. platform engineer" /></label>}<label>Posted within<select value={lookback} onChange={(event) => setLookback(event.target.value)}><option value="86400">Last 24 hours</option><option value="604800">Last 7 days</option><option value="2592000">Last month</option></select></label><label className={countryExcludesResults ? "location-filter is-blocking" : "location-filter"}>Location<select value={country} onChange={(event) => setCountry(event.target.value)} aria-describedby={countryExcludesResults ? "location-filter-note" : undefined}><option>India</option><option value="">All countries</option></select>{countryExcludesResults && <small id="location-filter-note">{jobsAcrossCountries} listings in All countries</small>}</label><button className="sync" type="button" onClick={searchAll} disabled={syncing}>Search</button></section>

...
export async function POST(request: Request, { params }: { params: Promise<{ source: string }> }) {
  ...
  const sync = source === "remotive" ? () => syncRemotive(keywords) : source === "linkedin" ? () => syncLinkedIn(keywords, location, validLookback) : source === "wellfound" ? () => syncWellfound(keywords) : source === "indeed" || source === "glassdoor" || source === "google" ? () => syncJobSpy(source, keywords, validLookback) : null;
